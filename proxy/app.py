...
def print_to_system(file_name, system_printer_name):
    ...
    try:
        ...
        print_command = ['lp', '-d', system_printer_name, '-o', 'fit-to-page', file_name]
...
def print_to_qz_tray(receipt_data):
    ...
    """
    print(f"⚠️ QZ Tray: Job received. QZ Tray communication is complex and requires a separate secure WebSocket implementation.")
    print("If QZ Tray runs on the same machine as the proxy, implement WSS client logic here.")
...
def detect_printers():
    ...
    printers["POS_Printer"] = {"system_name": "POS_Printer_Default", "type": "system", "kitchen": False}
    printers["Kitchen_Printer"] = {"system_name": "Kitchen_Printer_Default", "type": "system", "kitchen": True}
...
def assign_printer():
    ...
    printers[data['name']] = {
        ...
        "system_name": data.get("system_name", data['name']), # Used for 'system' type
...
def handle_default_printer_action():
    ...
    try:
        ...
        if not receipt_data:
            return jsonify({
                "jsonrpc": "2.0", "id": rpc_id, 
                "error": {"code": 100, "message": "Receipt data not found in payload."}
            }), 400
        ...
        if target_printer['type'] in ['system', 'escpos']:
            try:
                ...
                return jsonify({"jsonrpc": "2.0", "id": rpc_id, "error": {"code": 200, "message": f"Image decoding error: {e}"}}), 500

        # --- 3. ROUTING AND PRINTING ---
        if target_printer['type'] == 'system':
            # Decode, Save, and Print to OS Queue
            img = Image.open(BytesIO(image_bytes))
            file_name = os.path.join(IMAGE_SAVE_PATH, f'receipt_{rpc_id}_{printer_name}.png')
            img.save(file_name)
            print_success, print_message = print_to_system(file_name, target_printer.get("system_name"))
        ...
        if print_success:
            ...
            return jsonify({"jsonrpc": "2.0", "id": rpc_id, "error": {"code": 301, "message": print_message}}), 500
        ...
        return jsonify({"jsonrpc": "2.0", "id": rpc_id, "error": {"code": 300, "message": f"Proxy server error: {e}"}}), 500
